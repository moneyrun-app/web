...
export default function BookPage() {
  ...
  const { data: monthlyReports, isLoading: monthlyLoading } = useMonthlyReports(tab === 'detailed');
  const { data: wrongNotes, isLoading: wrongLoading } = useWrongNotes(tab === 'wrong');
  const { data: scraps, isLoading: scrapsLoading } = useScraps(tab === 'scrap');
  const createScrap = useCreateScrap();
  const [expandedNote, setExpandedNote] = useState<string | null>(null);
  const scrapDialogRef = useFocusTrap<HTMLDivElement>();
  ...
  return (
    <div className="relative">
            ...
            <div className="space-y-3">
              ...
              {(wrongNotes ?? []).map((note) => (
                <button
                  ...
                  className="w-full text-left bg-background border border-border rounded-2xl p-4 md:p-5 shadow-sm cursor-pointer"
                >
                  <p className="text-3xs text-placeholder mb-1">{note.category} · {note.source}</p>
                  <p className="text-sm font-medium text-foreground mb-2">{note.question}</p>

                  <div className="flex gap-2 mb-2">
                    <span className="inline-flex items-center gap-1 text-3xs px-2 py-0.5 rounded-full bg-grade-red-bg text-grade-red-text font-medium">
                      내 답: {note.choices?.[note.userAnswer - 1] ?? '—'}
                    </span>
                    <span className="inline-flex items-center gap-1 text-3xs px-2 py-0.5 rounded-full bg-grade-green-bg text-grade-green-text font-medium">
                  ...
                  {note.detailedExplanation && (
                    <p className="text-3xs text-accent mt-2">{expandedNote === note.id ? '접기' : '자세히 보기'}</p>
                  )}
                </button>
              ))}
            </div>
      ...
      <button
        ...
        className="fixed right-4 bottom-24 md:bottom-8 md:right-8 w-12 h-12 rounded-full bg-foreground text-background shadow-lg flex items-center justify-center hover:opacity-90 transition-opacity z-30"
      >
        <Plus size={22} />
      </button>
      ...
      {/* Scrap Modal */}
      {showScrapModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="scrap-title" ref={scrapDialogRef}>
          <div className="bg-background w-full md:w-[480px] rounded-t-2xl md:rounded-2xl p-5 md:p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <Link2 size={18} />
                <h2 id="scrap-title" className="text-lg font-bold">URL 스크랩</h2>
              </div>
            ...
            <label className="block mb-4">
              ...
              <input
                ...
                placeholder="https://..."
                className="w-full h-12 px-4 bg-surface border border-border rounded-xl text-foreground placeholder:text-placeholder focus:outline-none transition-shadow"
              />
            </label>
            <button
              onClick={handleScrap}
